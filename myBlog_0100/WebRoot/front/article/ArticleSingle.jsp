	...
	<body>
		<center>
			<table width="778" height="600" border="0" cellspacing="0"
						...
						<% 
	               			...
	               			if(reviewlist==null||reviewlist.size()==0){
                      			...
                      			out.print("<tr height='60'><td align='center'>【文章评论 共 "+num+" 条】</td></tr>");                            
               					int i=0;
               					//session.setAttribute("reviewlist",reviewlist);//把整个review都传递到reply页面
				...
				<tr height="50">
					<td style="text-indent: 20">
						...
						if(reviewSingle.getReAuthor().equals("kevin_lee")){
							...
							AuthorName = "用户："+reviewSingle.getReAuthor();
						}
						%>
						<%=AuthorName %>
						</b>
				...
				<%}else { %>
				<tr height="5">
				<!-- 明天用request.setAttribute("aa",fb)试试; -->
				<%
					//下面两个方法要连用才能传值成功，但是页面由于转发被跳转了，所以方法行不通
					//request.setAttribute("reviewBean",reviewSingle);
					//request.getRequestDispatcher("ArticleSingle.jsp").forward(request, response);
						
				 %>
					<td colspan="2" align="right"><a href="<%=basePath %>front/article/reply.jsp?reviewId=<%=i%>" class="topA">回复</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
				</tr>
				<%} %>
				<%
                    	       		i++;
