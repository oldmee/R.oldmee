		...
		<center>
			<table width="778" border="0" cellspacing="0" cellpadding="0"
				...
				<tr>
					...
					+ path + "/"%>ArticleAction?pageNo=0&master_id=<%=request.getAttribute("master_id") %>"
										method="post">
										<input type="hidden" name="action" value="adminSelectList_admin">
										文章类别：
										<select name="typeId">
											<option value=""></option>
											<%
							...
							<tr height="35">
								<td style="text-indent: 20">
									<a
										href="ArticleAction?action=adminSelectSingle_admin&id=<%=single.getArticle_id()%>"
								...
								<td align="center">
									<a
										href="ArticleAction?action=delete&pageNo=<%=pageNo %>&master_id=<%=request.getAttribute("master_id") %>&id=<%=single.getArticle_id()%>&typeId=<%=single.getArticleType().getArticleType_id()%>"
							...
							<tr>
								<td align="right">
								<a href="ArticleAction?action=adminSelectList_admin&typeId=<%=type_id %>&pageNo=0&master_id=<%=request.getAttribute("master_id")%>">第一页</a>&nbsp;
								<a href="ArticleAction?action=adminSelectList_admin&typeId=<%=type_id %>&pageNo=<%=pageNo-PAGE_SIZE %>&master_id=<%=request.getAttribute("master_id")%>">上一页</a>&nbsp;
								</td>
								
								<td align="left">&nbsp;
								<a href="ArticleAction?action=adminSelectList_admin&typeId=<%=type_id %>&pageNo=<%=pageNo+PAGE_SIZE %>&master_id=<%=request.getAttribute("master_id")%>">下一页</a>&nbsp;
								<a href="ArticleAction?action=adminSelectList_admin&typeId=<%=type_id %>&pageNo=<%=articlelist.size()-PAGE_SIZE %>&master_id=<%=request.getAttribute("master_id")%>">最末页</a>&nbsp;
								</td>
							</tr>
							<%
								}
